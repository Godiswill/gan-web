...
function imageBitmapToImageData(imageBitmap: ImageBitmap): ImageData {
  ...
  (ctx as OffscreenCanvasRenderingContext2D).drawImage(imageBitmap, 0, 0);
  return (ctx as OffscreenCanvasRenderingContext2D).getImageData(
    0,
    0,
    canvas.width,
    canvas.height
...
async function imageSourceToImageData(
  ...
  if (image instanceof ArrayBuffer || ArrayBuffer.isView(image)) {
    image = new Blob([image as BlobPart]);
  }
